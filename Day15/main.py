...
class Grid(dict):
    ...
    def move(self, unit, elf_death=False):
        targets = [
            ...
            if unit.team != target.team and target.alive
        ]
        occupied = set(
            u2.position for u2 in self.units if u2.alive and unit != u2
        )
        opponents = {target.position: target for target in targets}
        ...
        if any(nb in opponents for nb in unit.position.nb4):
            target = min(
                (opponents[nb] for nb in unit.position.nb4 if nb in opponents),
                key=lambda unit: (unit.hp, unit.position),
            )
...
print(f"Outcome of the combat: {Grid(puzzle).play()}")

for power in itertools.count(4):
    try:
        outcome = Grid(puzzle, power).play(elf_death=True)
    except ElfDied:
        continue
    else:
        print(f"Outcome of the combat after increasing the Elves' attack power: {outcome}")
        break
